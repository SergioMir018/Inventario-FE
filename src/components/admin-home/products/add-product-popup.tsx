...
export default function AddProductPopup() {
  ...
  const insertNewProductAction: SubmitHandler<IInsertNewProductForm> = async (data: IInsertNewProductForm) => {
    try {
      const requestData = {
        name: data.name,
  ...
  return (
    <div className="absolute w-screen h-screen bg-black/70 z-30 flex justify-center items-center text-white font-gabarito">
      <div className=" w-[35rem] bg-dark rounded-md flex flex-col justify-between">
        ...
        <form className="w-full h-[95%] px-5 pb-5 flex flex-col" onSubmit={handleSubmit(insertNewProductAction)}>
          <h1 className="text-2xl font-gabarito-bold">Agrega un nuevo producto</h1>
          <p className="text-white/60">Llena el formulario para agregar un nuevo producto a tu tienda</p>
          <label className="text-white font-gabarito pt-5">Nombre del producto</label>
          <input {...register("name")} type="text" className="w-full p-1 font-gabarito bg-transparent outline-none ring ring-white/50 focus:ring focus:ring-white mt-1 mb-2 rounded-sm" />
          <label className="text-white font-gabarito pt-5">Descripción corta</label>
          <textarea {...register("short_desc")} className="w-full h-15 resize-none p-1 font-gabarito bg-transparent outline-none ring ring-white/50 focus:ring focus:ring-white mt-1 mb-2 rounded-sm" />
          ...
          <div className="w-full h-60 flex flex-col gap-2">
            ...
            {file && <img src={fileScr} alt="Vista previa de la imagen" className="h-full w-full object-cover" />}
          </div>
