...
export default function SearchTimeDropDown() {
  ...
  return (
    <div ref={dropdownRef} className="flex flex-col absolute right-0 w-[25%]">
      <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="text-white bg-grey font-gabarito-medium hover:bg-darker focus:outline-none rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center justify-between" type="button" onClick={handleOnDropDownClick}>{period}<svg className="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
      ...
      { isOpen && (
        ...
        absolute pointer right-52 animate-fade-left animate-duration-100 animate-ease-in-out">
          <ul className="py-2 text-sm text-gray-700 dark:text-gray-200 font-gabarito-medium" aria-labelledby="dropdownDefaultButton">
            <li>
              <div onClick={() => handleDropdownClick("Todo el tiempo")} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer">
                Todo el tiempo
              </div>
            </li>
            <li>
