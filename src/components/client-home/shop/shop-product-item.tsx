...
export default function ShopProductsItem({ product }: ShopProductsItemProps) {
  ...
  const isProductInCart = cartContext?.cart.some(item => item.itemId === product.id);
      ...
      <div className="relative">
        ...
        <div className="w-full flex justify-center mt-2">
          <button
            ...
            className={`w-[90%] py-1 px-5 mb-3 text-black font-gabarito-bold rounded-md transition duration-100 ${isProductInCart ? 'bg-gray-400 cursor-not-allowed' : 'bg-white hover:bg-white/80'
              }`}
          >
