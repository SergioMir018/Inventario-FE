  ...
  return (
    <div className='w-full h-32 rounded-md bg-dark flex justify-between my-4'>
      ...
      <div className='w-72 mr-5 h-full flex justify-between items-center'>
        <button onClick={() => handleIncrementQuantity(cartItem.itemId)} className='border border-white/60 w-12 h-10 py-1 text-2xl font-gabarito-bold rounded-md hover:bg-white hover:text-black transition duration-100'>+</button>
        <p className='font-gabarito-medium text-2xl'>{cartItem.quantity}</p>
        <button onClick={() => handleDecrementQuantity(cartItem.itemId)} className='border border-white/60 w-12 h-10 py-1 text-2xl font-gabarito-bold rounded-md hover:bg-white hover:text-black transition duration-100'>-</button>
        <button onClick={() => handleCartItemDelete(cartItem.itemId)} className='border border-white/60 w-16 h-10 flex justify-center items-center rounded-md hover:bg-white hover:text-red transition duration-100'><TrashIcon /></button>
      </div>
    </div>
  );
}
